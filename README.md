...
Dockerized Run (Port 8090)
------

This repository now includes a production-ready Docker image and compose setup.

1. (Optional) set custom `APP_KEY` in your shell. If you skip this, container will auto-generate a key at startup.
2. Run:
...
- `app` (Laravel + Apache on container port `8090`)
- `mysql` (MySQL 8.0 on host port `3306`)
...
- On `pull_request` to `main`: builds image (validation only, no push)
- On `push` to `main` or `v*` tag: builds and pushes image to GHCR
...
To pull and run published image manually:

```bash
docker run --rm -p 8090:8090 ghcr.io/<owner>/<repo>:latest
```
